...
def regret_plotting(regret, cases, plotting_info):
    ...
    for c in range(cases):
        ...
        plt.plot(horizon, batched_regret, colors[c] + shape[c], label=r'$Q=$'+str(plotting_info[4][c]))
...
def getSamples(data_file_name, samples, generate_new_data, save_generated_data, random_seed):
    ...
    if generate_new_data:
        ...
        for l in range(arms):
            observations[:, l] = np.random.binomial(1, mu_vector[l], size=samples)
...
# file_to_save        = "sameThetaQ_" + str(arms) + "_" + str(num_cases) + "_" + str(horizon) + "_" + str(runs) + ".png"
file_to_save        = "sameThetaQ.png"
title               = "Varying Resources"
save_to_path        = "plots/" 
